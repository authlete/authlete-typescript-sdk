 ...
 */
export function backChannelLogoutBackchannelLogoutTokenApi(
  client: AuthleteCore,
  request: operations.BackchannelLogoutTokenApiRequest,
  options?: RequestOptions,
): APIPromise<
  Result<
    operations.BackchannelLogoutTokenApiResponse,
    | errors.ResultError
    | AuthleteError
...
> {
  const parsed = safeParse(
    request,
    (value) =>
      operations.BackchannelLogoutTokenApiRequest$outboundSchema.parse(value),
  ...
  const body = encodeJSON("body", payload.backchannel_logout_token_request, {
    explode: true,
  });
  ...
  const path = pathToFunc("/api/{serviceId}/backchannel/logout/token")(
    pathParams,
  );
  ...
  const [result] = await M.match<
    ...
    | SDKValidationError
  >(
    M.json(200, operations.BackchannelLogoutTokenApiResponse$inboundSchema, {
      key: "Result",
    }),
